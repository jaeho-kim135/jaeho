...
public final class SparkMultiQueryNodeDialog extends DataAwareNodeDialogPane {
    ...
    // --- Expression templates (91 presets) ---
    private static final String[][] TEMPLATES = {
        ...
        {"Left Trim", "LTRIM(" + PH + ")"},
        {"Right Trim", "RTRIM(" + PH + ")"},
        {"String Length", "LENGTH(" + PH + ")"},
        {"Capitalize Words", "INITCAP(" + PH + ")"},
        {"Reverse String", "REVERSE(" + PH + ")"},
        {"Concat", "CONCAT(" + PH + ", '')"},
        {"Concat with Separator", "CONCAT_WS(',', " + PH + ", '')"},
        {"Substring", "SUBSTRING(" + PH + ", 1, 5)"},
        {"Substring by Delimiter", "SUBSTRING_INDEX(" + PH + ", ',', 1)"},
        {"Replace", "REPLACE(" + PH + ", '', '')"},
        {"Translate Characters", "TRANSLATE(" + PH + ", 'abc', 'xyz')"},
        {"Left Pad", "LPAD(" + PH + ", 10, '0')"},
        {"Right Pad", "RPAD(" + PH + ", 10, ' ')"},
        {"Find Position (INSTR)", "INSTR(" + PH + ", '')"},
        {"Split to Array", "SPLIT(" + PH + ", ',')"},
        {"Repeat String", "REPEAT(" + PH + ", 2)"},
        {"Format Number", "FORMAT_NUMBER(" + PH + ", 2)"},
        // Regular Expression
        {"Regex Replace (non-digits)", "REGEXP_REPLACE(" + PH + ", '[^0-9]', '')"},
        {"Regex Extract", "REGEXP_EXTRACT(" + PH + ", '(.*)', 1)"},
        {"Regex Match (RLIKE)", PH + " RLIKE ''"},
        {"SQL Pattern (LIKE)", PH + " LIKE '%'"},
        // Math
        {"Round to 2 decimals", "ROUND(" + PH + ", 2)"},
        {"Absolute Value", "ABS(" + PH + ")"},
        {"Ceiling", "CEIL(" + PH + ")"},
        {"Floor", "FLOOR(" + PH + ")"},
        {"Modulo", "MOD(" + PH + ", 2)"},
        {"Square Root", "SQRT(" + PH + ")"},
        {"Power", "POW(" + PH + ", 2)"},
        {"Sign (+1/0/-1)", "SIGN(" + PH + ")"},
        {"Log Base 10", "LOG10(" + PH + ")"},
        {"Natural Log (ln)", "LN(" + PH + ")"},
        {"Exponential (e^x)", "EXP(" + PH + ")"},
        {"Greatest (max of values)", "GREATEST(" + PH + ", 0)"},
        {"Least (min of values)", "LEAST(" + PH + ", 0)"},
        // Date/Time
        {"Parse Date (yyyyMMdd)", "TO_DATE(string(" + PH + "), 'yyyyMMdd')"},
        {"Parse Timestamp", "TO_TIMESTAMP(string(" + PH + "), 'yyyy-MM-dd HH:mm:ss')"},
        {"Format Date/Time", "DATE_FORMAT(" + PH + ", 'yyyy-MM-dd')"},
        {"Days Difference from Today", "DATEDIFF(CURRENT_DATE(), " + PH + ")"},
        {"Add Days", "DATE_ADD(" + PH + ", 1)"},
        {"Subtract Days", "DATE_SUB(" + PH + ", 1)"},
        {"Add Months", "ADD_MONTHS(" + PH + ", 1)"},
        {"Months Between", "MONTHS_BETWEEN(CURRENT_DATE(), " + PH + ")"},
        {"Last Day of Month", "LAST_DAY(" + PH + ")"},
        {"Truncate to Month", "DATE_TRUNC('month', " + PH + ")"},
        {"Extract Year", "YEAR(" + PH + ")"},
        {"Extract Month", "MONTH(" + PH + ")"},
        {"Extract Quarter", "QUARTER(" + PH + ")"},
        {"Extract Day of Month", "DAYOFMONTH(" + PH + ")"},
        {"Extract Day of Week", "DAYOFWEEK(" + PH + ")"},
        {"Extract Day of Year", "DAYOFYEAR(" + PH + ")"},
        {"Extract Week of Year", "WEEKOFYEAR(" + PH + ")"},
        {"Extract Hour", "HOUR(" + PH + ")"},
        {"Extract Minute", "MINUTE(" + PH + ")"},
        {"Extract Second", "SECOND(" + PH + ")"},
        {"To Unix Timestamp", "UNIX_TIMESTAMP(" + PH + ")"},
        {"From Unix Timestamp", "FROM_UNIXTIME(" + PH + ", 'yyyy-MM-dd HH:mm:ss')"},
        // Null Handling
        {"Replace NULL with 0", "COALESCE(" + PH + ", 0)"},
        {"Replace NULL with empty string", "COALESCE(" + PH + ", '')"},
        {"IFNULL (default if null)", "IFNULL(" + PH + ", '')"},
        {"NVL (default if null)", "NVL(" + PH + ", '')"},
        {"NVL2 (by null status)", "NVL2(" + PH + ", 'has value', 'null')"},
        {"NULLIF (null if equal)", "NULLIF(" + PH + ", '')"},
        {"Is Null check", "ISNULL(" + PH + ")"},
        {"Is Not Null check", "ISNOTNULL(" + PH + ")"},
        {"Is NaN check", "ISNAN(" + PH + ")"},
        {"Replace NaN with default", "NANVL(" + PH + ", 0)"},
        // Type Cast
        {"Cast to String", "string(" + PH + ")"},
        {"Cast to Integer", "CAST(" + PH + " AS INT)"},
        {"Cast to Double", "CAST(" + PH + " AS DOUBLE)"},
        {"Cast to BigInt (Long)", "CAST(" + PH + " AS BIGINT)"},
        {"Cast to Float", "CAST(" + PH + " AS FLOAT)"},
        {"Cast to Boolean", "CAST(" + PH + " AS BOOLEAN)"},
        {"Cast to Decimal(10,2)", "CAST(" + PH + " AS DECIMAL(10,2))"},
        {"Cast to Date", "CAST(" + PH + " AS DATE)"},
        {"Cast to Timestamp", "CAST(" + PH + " AS TIMESTAMP)"},
        // Conditional
        {"IF condition", "IF(" + PH + " > 0, 'positive', 'non-positive')"},
        {"CASE WHEN", "CASE WHEN " + PH + " IS NULL THEN 'null' ELSE " + PH + " END"},
        {"Type Of", "TYPEOF(" + PH + ")"},
        // Hash / Encoding
        {"MD5 Hash", "MD5(string(" + PH + "))"},
        {"SHA-256 Hash", "SHA2(string(" + PH + "), 256)"},
        {"Base64 Encode", "BASE64(" + PH + ")"},
        {"Base64 Decode", "UNBASE64(" + PH + ")"},
        // JSON
        {"Get JSON Value", "GET_JSON_OBJECT(" + PH + ", '$.key')"},
        {"To JSON String", "TO_JSON(" + PH + ")"},
        {"JSON Array Length", "JSON_ARRAY_LENGTH(" + PH + ")"},
        // Array
        {"Array Contains", "ARRAY_CONTAINS(" + PH + ", 'value')"},
        {"Array Size", "ARRAY_SIZE(" + PH + ")"},
        {"Array Element At", "ELEMENT_AT(" + PH + ", 1)"},
    };
