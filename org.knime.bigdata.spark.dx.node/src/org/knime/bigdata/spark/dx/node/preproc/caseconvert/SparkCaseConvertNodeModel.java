...
/**
 * Node model for the Spark Case Converter node.
 * Converts the case of selected string columns using UPPER(), LOWER(), or INITCAP().
 */
public class SparkCaseConvertNodeModel extends SparkNodeModel {

    /** The unique Spark job id. */
    public static final String JOB_ID = SparkCaseConvertNodeModel.class.getCanonicalName();

    private final SparkCaseConvertSettings m_settings = new SparkCaseConvertSettings();

    /** Constructor. */
    public SparkCaseConvertNodeModel() {
    ...
    protected PortObjectSpec[] configureInternal(final PortObjectSpec[] inSpecs)
        ...
        if (columns == null || columns.isEmpty()) {
            setWarningMessage("No columns selected for case conversion.");
        } else {
        ...
        // Output spec is the same as input -- StringType columns remain StringType
    ...
    protected PortObject[] executeInternal(final PortObject[] inData, final ExecutionContext exec)
        ...
        final List<String> columns = m_settings.getColumns();
        if (columns == null || columns.isEmpty()) {
            // No columns selected -- pass through
            return new PortObject[]{sparkPort};
        }

        final SparkCaseConvertJobInput jobInput = new SparkCaseConvertJobInput(
            inputObject, outputObject,
            columns.toArray(new String[0]),
            m_settings.getMode());

        exec.setMessage("Executing Spark case conversion...");
        final SparkCaseConvertJobOutput jobOutput = SparkContextUtil
            .<SparkCaseConvertJobInput, SparkCaseConvertJobOutput>getJobRunFactory(contextID, JOB_ID)
            .createRun(jobInput)
            .run(contextID, exec);
