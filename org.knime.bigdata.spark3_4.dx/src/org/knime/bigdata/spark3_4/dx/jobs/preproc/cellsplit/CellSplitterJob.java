...
public class CellSplitterJob implements SparkJob<SparkCellSplitterJobInput, SparkCellSplitterJobOutput> {
    ...
    public SparkCellSplitterJobOutput runJob(final SparkContext sparkContext, final SparkCellSplitterJobInput input,
        ...
        for (int i = 0; i < numCols; i++) {
            // Guard with size check to avoid ANSI mode ArrayIndexOutOfBoundsException
            // (try_element_at is Spark 3.5+ only; Spark 3.4 uses when/element_at)
            Column elem = when(size(col(tmpSplitCol)).geq(lit(i + 1)),
                element_at(col(tmpSplitCol), lit(i + 1)));
            if (input.isTrim()) {
                elem = trim(elem);
            }
